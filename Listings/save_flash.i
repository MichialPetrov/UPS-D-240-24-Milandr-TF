...
Status Check_Update_ID_Device(void)
{
 ...
 if ((memcmp (UPS_D.Device_ID.Device_Name, 		 UPS_D_buf.Device_ID.Device_Name, 		 11) 			!=NO)	||
		 (memcmp (UPS_D.Device_ID.Firmware_Version,UPS_D_buf.Device_ID.Firmware_Version, 6) !=NO)	||
		 (memcmp (UPS_D.Device_ID.Order_Number, 	 UPS_D_buf.Device_ID.Order_Number, 		 9) 		!=NO)	||
     (memcmp (UPS_D.Device_ID.Batch_Number, 	 UPS_D_buf.Device_ID.Batch_Number, 		 8) 		!=NO)	||
     (memcmp (UPS_D.Device_ID.Number_In_Batch, UPS_D_buf.Device_ID.Number_In_Batch,  1) 	!=NO))
	  {
			 return YES;
		}
	else return NO;
...
void Write_New_ID_Device_To_Flash(void)
{
	...
	Address=Save_ID_Device(UPS_D_buf.Device_ID.Device_Name, 		 11, 			Address);
	Address=Save_ID_Device(UPS_D_buf.Device_ID.Firmware_Version, 6, Address);
	Address=Save_ID_Device(UPS_D_buf.Device_ID.Order_Number,		 9, 		Address);
	Address=Save_ID_Device(UPS_D_buf.Device_ID.Batch_Number, 		 8, 		Address);
	Address=Save_ID_Device(UPS_D_buf.Device_ID.Number_In_Batch,  1, 	Address);
 __enable_irq();	 
}
...
void Write_New_Settings_To_Flash(void)
{
	...
	{			
		...
		switch (i)
		{
			...
			case 4:	 Data = (uint32_t)(UPS_D_buf.U.Load.Min_Value 						* (float)100.);		break;
			case 5:  Data = (uint32_t)(UPS_D_buf.U.Load.Max_Value 						* (float)100.);		break;
			case 6:  Data = (uint32_t)(UPS_D_buf.U.Battery.Min_Value 					* (float)100.);		break;
			case 7:  Data = (uint32_t)(UPS_D_buf.U.Battery.Max_Value 					* (float)100.);		break;
			case 8:  Data = (uint32_t)(UPS_D_buf.U.KAN_D.Min_Value 						* (float)100.);		break;
			case 9:  Data = (uint32_t)(UPS_D_buf.U.KAN_D.Max_Value 						* (float)100.);		break;
			...
			case 13: Data = (uint32_t)(UPS_D_buf.U_Tricle 										* (float)100.);		break;
			case 14: Data = (uint32_t)(UPS_D_buf.U_Over 											* (float)100.);		break;
			case 15: Data = (uint32_t)(UPS_D_buf.U_Equalizing 								* (float)100.);		break;
			case 16: Data = (uint32_t)(UPS_D_buf.U_Float 											* (float)100.);		break;
...
uint32_t Convert_Temperature_To_Save_Flash(float Temperature)
{
		...
		if(Temperature<0)	
			{
				data = (-1)*Temperature*(float)100.;	
				data1=(uint32_t)data;		
				data1=~(data1-1);				
			}
			else 
			{
				data = Temperature*(float)100.;
				data1=(uint32_t)data;		
			}
		return data1;							
}
