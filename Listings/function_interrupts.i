...
{
	((MDR_PORT_TypeDef *) (0x400E8000))->RXTX|=(int)LED ->pin[LED_OFF]; 
	
	if((LED -> Flash&(1<<LED -> FlashKvantCount))>>LED -> FlashKvantCount==1)
	{
		
		((MDR_PORT_TypeDef *) (0x400E8000))->RXTX&=(int)LED ->pin[LED ->Color];
	}
	else
	{
 ...
 if(LED -> FlashKvantPeriodCount==((1000/8)*9000/1000))
	{
		LED -> FlashKvantPeriodCount=0;
		(LED -> FlashKvantCount==8-1)?(LED -> FlashKvantCount=0):(LED -> FlashKvantCount++);
	}
...
void ADC_Calculate(ADCcontrol *ADC)
{
	...
	((MDR_ADC_TypeDef *) (0x40088000))->ADC1_CFG&=~((0x1F)<<4);
  
	((MDR_ADC_TypeDef *) (0x40088000))->ADC1_CFG|=(ADC ->Channel);
	
	ADC1_Start();	
	
	uint16_t count=0;
	while ((ADC1_GetStatus() & ((uint32_t)0x00000004)) == 0) if(++count==0xA) return;
  
	ADC->adc_data[ADC->index] = ADC1_GetResult();
  
	ADC->adc_data_sum+=ADC->adc_data[ADC->index];
	
	if(ADCready_to_calculate==0xBF)
	{ 
		ADC->avr=ADC->adc_data_sum/10;
	 ...
	 ADC_avr_calculate_end|=YES<<(ADC ->Channel>>4);
	}
  
  (ADC->index==10-1)?(ADCready_to_calculate|=YES<<(ADC ->Channel>>4)):(ADCready_to_calculate|=ADCready_to_calculate);
	
	(ADC->index==10-1)?(ADC->adc_data_sum-=ADC->adc_data[0]):(ADC->adc_data_sum-=ADC->adc_data[ADC->index+1]);
	
	(ADC->index==10-1)?(ADC->adc_data[0]=0):(ADC->adc_data[ADC->index+1]=0);
	
	(ADC->index==10-1)?(ADC->index=0):(ADC->index++);
}
...
void Check_UPS_Charge_To_Discharge(void)
{
		...
		(Counter_Transient_Process_UPS.Status==OFF)) 
	{
		 ...
		 Battery.UPS_Mode_Has_Changed_Flag = UPS_FLAG_CHARGE_TO_DISCHARGE; 
		 Counter_Transient_Process_UPS.Status=ON;	 
		 Battery.Counter_UnderCharge.Status=ON;	 
		 if(MODE_CHARGE!=FLOAT)	Counter_Handling (&Battery.Counter_UnderCharge);  
	}
}
...
void Check_UPS_Discharge_To_Charge(void)
{
	if((MODE==DISCHARGE)&&(ADC4_U_KAN_D.avr_real>UPS_D.U.KAN_D.Min_Value+U_Hysteresis_KAN_D())&&(Counter_Transient_Process_UPS.Status==OFF))
	{
	 ...
	 Battery.UPS_Mode_Has_Changed_Flag = UPS_FLAG_DISCHARGE_TO_CHARGE; 
	 Counter_Transient_Process_UPS.Status=ON; 
	}
}
...
void Check_UPS_Alarm_To_Discharge(void)
{
	if((MODE==ALARM)&&
		 ((Wrng_U_KAN_D.WarningStatus==YES)||(Wrng_U_LOAD.WarningStatus==YES)||((ADC4_U_KAN_D.avr_real<UPS_D.U.KAN_D.Min_Value)&&(device_status[U_KAN_D_MORE_U_BATTERY_DISCHARGE_MODE]!=YES)))&&
			((device_status[I_BATTERY_LESS_MIN]==YES)	||													
			((device_status[T_BATTERY_LESS_MIN]==YES)&&(MODE_prev==CHARGE)))&&	 	
			(Counter_Transient_Process_UPS.Status==OFF)) 
	{
		...
		Rele_Status(&RELE2_BATTERY);
		device_status[U_KAN_D_LESS_MIN]=YES;
		Battery.UPS_Mode_Has_Changed_Flag = UPS_FLAG_ALARM_TO_DISCHARGE; 
		Counter_Transient_Process_UPS.Status=ON;	 
		Battery.Counter_UnderCharge.Status=ON;	 
		if(MODE_CHARGE!=FLOAT)	Counter_Handling (&Battery.Counter_UnderCharge);  
	}
...
void Check_I_Battery_Discharge(void)
{
	if((MODE==DISCHARGE)&&(ADC1_I_BATTERY.avr_real>UPS_D.I.Battery.Max_Value_Discharge*(float)1.1)&&(Counter_Transient_Process.Status==OFF)) 
	{
		MODE=ALARM;MODE_prev=DISCHARGE;
		{if(OFF==YES){ ((MDR_PORT_TypeDef *) (0x400B0000))->RXTX = PORT_Pin_8|(((MDR_PORT_TypeDef *) (0x400B0000))->RXTX&0xFFE0); Battery . Status_Join_To_Load = ON;} else { ((MDR_PORT_TypeDef *) (0x400B0000))->RXTX&= ~(PORT_Pin_8|0x001F); Battery . Status_Join_To_Load = OFF;}};												 
		Battery.I_Battery_Discharge_More_Max_Value=YES;		 
		Battery.Counter_Check_I_Battery.Status=ON;  
		device_status[I_BATTERY_MORE_MAX]=YES;
	}
	if((MODE==ALARM)&&(ADC1_I_BATTERY.avr_real>UPS_D.I.Battery.Max_Value_Discharge*(float)1.1)) {if(OFF==YES){ ((MDR_PORT_TypeDef *) (0x400B0000))->RXTX = PORT_Pin_8|(((MDR_PORT_TypeDef *) (0x400B0000))->RXTX&0xFFE0); Battery . Status_Join_To_Load = ON;} else { ((MDR_PORT_TypeDef *) (0x400B0000))->RXTX&= ~(PORT_Pin_8|0x001F); Battery . Status_Join_To_Load = OFF;}};
}
