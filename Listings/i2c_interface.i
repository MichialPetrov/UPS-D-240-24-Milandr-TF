...
#line 4 "I2C_interface.c"
#line 1 "I2C_interface.h"


void delays(void);
char OW_Write_Byte(unsigned char Data); 
char OW_Reset(void); 					
char OW_Presence(void);  			
void Check_OW_Presence(void);	

char I2C_Device_Reset(void); 
char I2C_Read(void);


void Read_Temperature(void);		
char Handler_Temperature(void);	
char Save_I2C_Recieved_Data();   
...
void Read_Temperature(void)
{
	if(Handler_Temperature()==FAULT)
	{
		...
		if (I2C_OW_Converter.count_Fault_Connect.Flag==YES)	I2C_OW_Converter.Fault_Connect=YES; 
	}
	else { I2C_OW_Converter . Fault_Connect=NO; I2C_OW_Converter . count_Fault_Connect . Count=0; };
}
...
char I2C_OW_Converter_Reset(void)
{
	...
	{ while(!I2C_CheckEvent((((I2C_FLAG_BUS_BUSY | (I2C_FLAG_LOST_ARB << 16)) | (I2C_FLAG_TRANS << 16) | (I2C_FLAG_CMD_WR << 16) | (I2C_FLAG_CMD_RD << 16)) | (I2C_FLAG_SLAVE_NACK << 16)))){ I2C_OW_Converter . Counter_ACK_Flag . Status=ON; if(I2C_OW_Converter . Counter_ACK_Flag . Flag==YES) return FAULT;} { I2C_OW_Converter . Counter_ACK_Flag . Count=0; I2C_OW_Converter . Counter_ACK_Flag . Flag=NO; I2C_OW_Converter . Counter_ACK_Flag . Status=OFF;};};
...
char I2C_Read(void)
{
  ...
  I2C_SendByte(0xE1);
	...
	I2C_Send7bitAddress(0x33,I2C_Direction_Receiver);
	{ while(!I2C_CheckEvent((((I2C_FLAG_BUS_BUSY | (I2C_FLAG_LOST_ARB << 16)) | (I2C_FLAG_TRANS << 16) | (I2C_FLAG_CMD_WR << 16) | (I2C_FLAG_CMD_RD << 16)) | (I2C_FLAG_SLAVE_NACK << 16)))){ I2C_OW_Converter . Counter_ACK_Flag . Status=ON; if(I2C_OW_Converter . Counter_ACK_Flag . Flag==YES) return FAULT;} { I2C_OW_Converter . Counter_ACK_Flag . Count=0; I2C_OW_Converter . Counter_ACK_Flag . Flag=NO; I2C_OW_Converter . Counter_ACK_Flag . Status=OFF;};};
...
void Check_OW_Presence(void)
{
	if((I2C_GetReceivedData()& 0x02) != 0x02)
	{
		Counter_Handling (&TemperatureSensor1.count_Fault_Connect);
		
		if(TemperatureSensor1.count_Fault_Connect.Flag==YES) 	TemperatureSensor1.Fault_Connect=YES; 
	}
	else 	{ TemperatureSensor1 . Fault_Connect=NO; TemperatureSensor1 . count_Fault_Connect . Count=0; };
}
...
char Handler_Temperature(void)
{
	...
	OW_Presence(); 
	{ if(OW_Write_Byte(0xCC)==FAULT) return FAULT;};		
	{ if(OW_Write_Byte(0x44)==FAULT) return FAULT;};	
	
	{ if(OW_Reset()==FAULT) return FAULT;};								
	OW_Presence(); 
	{ if(OW_Write_Byte(0xCC)==FAULT) return FAULT;};		
	{ if(OW_Write_Byte(0xBE)==FAULT) return FAULT;};						
...
{
	if(TemperatureSensor->Fault_Connect!=YES) 
	{
		TemperatureSensor->Data[TemperatureSensor->Byte_number]=I2C_GetReceivedData();	
		TemperatureSensor->Temperature_code|=TemperatureSensor->Data[TemperatureSensor->Byte_number]<<(8*TemperatureSensor1.Byte_number); 

		if(TemperatureSensor->Byte_number==0x01) 
		{
			
			TemperatureSensor->Sign=((TemperatureSensor->Temperature_code & 0xF800) != 0xF800)?(POSITIVE):(NEGATIVE);
			if(TemperatureSensor->Sign==NEGATIVE) TemperatureSensor->Temperature_code = (~TemperatureSensor->Temperature_code)+1; 
	...
	{
		TemperatureSensor->Temperature_real=25;
